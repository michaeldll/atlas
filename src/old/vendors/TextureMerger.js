//Adapted from https://github.com/oguzeroglu/TextureMerger
...
class TextureMergerRectangle {
  ...
  set(x, y, x2, y2, width, height) {
    this.x = x
    this.y = y
    this.finalX = x2
    this.finalY = y2; (this.width = width), (this.height = height)
    return this
  }
  fits(texture) {
...
// CompressedTextures are not supported.
class TextureMerger {
  constructor(texturesObj, size) {
    this.MAX_TEXTURE_SIZE = size
    ...
    this.node.rectangle = new TextureMergerRectangle(0, 0, this.maxWidth * this.textureCount, this.maxHeight * this.textureCount)
    ...
    /*
      Make texture from canvas like this
  ...
  isTextureAlreadyInserted(textureName, texturesObj) {
    var texture = texturesObj[textureName]
    ...
    for (var tName in texturesObj) {
      ...
      if (img == tImg && txt.offset.x == texture.offset.x && txt.offset.y == texture.offset.y && txt.offset.z == texture.offset.z && txt.repeat.x == texture.repeat.x && txt.repeat.y == texture.repeat.y && txt.flipX == texture.flipX && txt.flipY == texture.flipY && txt.wrapS == texture.wrapS && txt.wrapT == texture.wrapT) {
  ...
  insert(node, textureName, texturesObj) {
    var texture = texturesObj[textureName]
  ...
  calculateImageSize(texturesObj) {
    var width = 0
    var height = 0
